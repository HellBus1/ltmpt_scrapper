...
fun generateUniversityListDocument() {
    ...
    val urlPathName = "src/resources/university_url_list_aggregates.txt"
...
//                    println("Data Aggregate : $dataAggregate")
                    ...
                    val fileName = "src/resources/universitydemand/${currentPtn}.json"
                    currentPtn = queryStringsMap["ptn"]!!
...
fun generateDetailUniverstyUrlList() {
    val driver: WebDriver = SafariDriver()
    val fileName = "src/resources/university_url_list_aggregates.txt"
    val file = File(fileName)
...
//            driver.manage().timeouts().implicitlyWait(4, TimeUnit.SECONDS)
            ...
            val saintekTableContents: List<WebElement> = driver.findElements(By.cssSelector("#jenis1 tbody tr"))
            val soshumTableContents: List<WebElement> = driver.findElements(By.cssSelector("#jenis2 tbody tr"))

            for (tr in saintekTableContents) {
                val tdList: List<WebElement> = tr.findElements(By.cssSelector("td")).map { it }
                urlList.add(tdList[2].findElement(By.cssSelector("a")).getAttribute("href").toString())
            }
            for (tr in soshumTableContents) {
                val tdList: List<WebElement> = tr.findElements(By.cssSelector("td")).map { it }
                urlList.add(tdList[2].findElement(By.cssSelector("a")).getAttribute("href").toString())
            }
            file.printWriter().use { out ->
